...
def main():
    ...
    if PLOTLY_AVAILABLE:
        ...
        if fig_3d:
            fig_3d.write_html('/workspace/mbr_simulation/mbr_3d_scene.html')
            print(f"✅ 3D场景已保存: mbr_3d_scene.html")
        
        # 剪切力分析
        shear = calc.calculate_shear_stress()
        fig_shear = viz.create_shear_profile_chart(shear)
        if fig_shear:
            fig_shear.write_html('/workspace/mbr_simulation/mbr_shear.html')
            print(f"✅ 剪切力分析已保存: mbr_shear.html")
        
        # 性能仪表盘
        fig_gauge = viz.create_performance_gauge(result)
        if fig_gauge:
            fig_gauge.write_html('/workspace/mbr_simulation/mbr_gauge.html')
            print(f"✅ 性能仪表盘已保存: mbr_gauge.html")
        
        print(f"\n📁 可视化文件:")
        print(f"   - mbr_3d_scene.html (统一3D场景)")
        print(f"   - mbr_shear.html (剪切力分析)")
        print(f"   - mbr_gauge.html (性能仪表盘)")
    ...
    with open('/workspace/mbr_simulation/mbr_report.json', 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"\n📄 报告已保存: mbr_report.json")
